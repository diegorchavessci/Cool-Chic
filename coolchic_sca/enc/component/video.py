        ...
        for mod_name, module in modulos.items():
            for param_name, param in module.named_parameters():
                ...
                #print(f"    - Intermediários  : {pesos_intermediarios:<7d} ({pct_interm:5.1f}%)")
                 ...
                 # Flatten para facilitar a contagem
                p = param.data.view(-1)
                total_pesos = p.numel()
                tol = 0.1

                # Máscaras para cada faixa
                prox_menos_um = torch.sum((p >= -1 - tol) & (p <= -1 + tol)).item()
                prox_zero     = torch.sum((p >= -tol) & (p <= tol)).item()
                prox_um       = torch.sum((p >= 1 - tol) & (p <= 1 + tol)).item()

                # Porcentagens
                pct_menos_um = 100 * prox_menos_um / total_pesos
                pct_zero     = 100 * prox_zero / total_pesos
                pct_um       = 100 * prox_um / total_pesos

                print(f"  Módulo: {mod_name} | Parâmetro: {param_name} | Shape: {list(param.shape)}")
                print(f"    - Próximos de -1 : {prox_menos_um:<7d} ({pct_menos_um:5.1f}%)")
                print(f"    - Próximos de  0 : {prox_zero:<7d} ({pct_zero:5.1f}%)")
                print(f"    - Próximos de  1 : {prox_um:<7d} ({pct_um:5.1f}%)")
